...
$(function(){//window.onload=function(){} 초기화 작업
    ...
    //로컬스토리지객체 생성 =>DB대용->프로파일러 목적(계정별로 서로다른 환경설정 불러옴)
    ...
    let fontSizeField=document.getElementById("fontSize"); //폰트

    //맨처음에 로그인했을떄 기본 배경색상, 글자크기
    let defaultBackground="#ffffff" ;
    let defaultFontSize=15;

    //1. storage에 저장
    $('#save').click(function(){//save.onclick=function(){}
        //로컬스토러지.serItem("키명",저장할값)<->getItem("키명")
        let user={
            name:nameField.value, /* 키명: 입력받은 값*/ 
            color:colorField.value,
            size:fontSizeField.value 
        }
        //로컬스토러지에 저장하기위해서는 객체 => 문자열로 변환 JSON.stringify(객체명)
